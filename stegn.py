...
class HuffmanCoding:
    ...
    def make_codes(self):
        ...
        current_code = ""        self.make_codes_helper(root, current_code)
...
def side0(pixel, channel, data, currRow):
    global columns, img    tRow = currRow
    if side0Mode == 0:
...
def side2(pixel, channel, data, currRow):
    global rows, columns, img
    tRow = rows - currRow - 1    if side2Mode == 0:
        tCol = columns - currRow - 2 - pixel
    else:
        tCol = currRow + 1 + pixel
    temp1 = img[tRow][tCol][channel] % 2
...
def embedEdge(currRow):
    ...
    sides = []
    bitsThisIter = 0    (b, g, r) = img[currRow, currRow]
    s = [getBinary(b), getBinary(g), getBinary(r)]
...
def embed():
    ...
    while keepLooping:
        ...
        keepLooping = embedEdge(currRow)        if embeddedBits >= len(compTextRef):
            keepLooping = False
        currRow += 1
...
def getSideData(currRow):
    ...
    side = getSide(b % 2, g % 2)
    sides.append(side)
    assignMode(side, r % 2)    (b, g, r) = img[rows - currRow - 1][columns - currRow - 1]
    side = getSide(b % 2, g % 2)
    sides.append(side)
    assignMode(side, r % 2)
    for i1 in range(4):
...
def getSide2(currRow, pixel, channel):
    global img, rows, columns    tRow = rows - currRow - 1
    if side2Mode == 0:
        tCol = columns - currRow - 2 - pixel
    else:
        tCol = currRow + 1 + pixel
...
encryptionIpCopy = encryptionIpencryptionIp = hex(int(encryptionIp, 2))[2:].zfill(int(len1))
...
embed()
cv2.imwrite('encrypted.PNG', img)cv2.imshow('encrypted image', img)
